...
(function($) {
    ...
    $.each(odds, function(index, odd){
        if(awayScoreByGameId[odd.gameId]==null){
        	awayScoreByGameId[odd.gameId]=Math.floor(Math.random()*20);
        }
        if(homeScoreByGameId[odd.gameId]==null){
        	homeScoreByGameId[odd.gameId]=Math.floor(Math.random()*20);
        }

        var pass = '不知道有沒有過關';    	
    	//console.log("odd:");
    	//console.log(odd);
    	$('#oddsList tr:eq('+index+') td:eq(6)').html(awayScoreByGameId[odd.gameId]+":"+homeScoreByGameId[odd.gameId]);  
        $('#oddsList tr:eq('+index+') td:eq(7)').html(odd.oddType);  
        ...
        $('#oddsList tr:eq('+index+') td:eq(8)').html(pass);
    });
